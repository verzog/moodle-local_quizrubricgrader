define(['jquery'], function($) {
    ...
    var RubricGrader = {
        ...
        init: function() {
            ...
            self.log('Added rs-grading-page class to body');
        ...
        setupTable: function($table) {
            ...
            $cells.each(function() { $(this).addClass('rs-clickable'); });
        ...
        injectSplitViewButton: function() {
            var self = this;
            if (document.getElementById('rs-split-btn')) return;
            ...
            var $btn = $('<button type="button" id="rs-split-btn" class="rs-split-toggle">&#9707; Split View &mdash; student response on left, mark on right</button>');
        ...
        injectRescaleWidget: function($table) {
            var self = this;
            // Don't add twice
            if ($table.next('.rs-rescale-wrap').length) return;

            var $widget = $([
                '<div class="rs-rescale-wrap">',
                '  <div class="rs-rescale-inner">',
                '    <span class="rs-rescale-label">&#9881; Rescale mark</span>',
                '    <span class="rs-rescale-hint">Enter the question\'s max mark, then click Rescale to convert the rubric total.</span>',
        ...
        writeToCommentArea: function($table, html) {
            ...
            $('textarea').each(function() {
                ...
                if (d < 1000) { $near = $(this); return false; }
            });
            if ($near && $near.length) {
        ...
        updateTotal: function() {
            ...
            var maxPossible = 0;
            var breakdown = [];
            var $selected = $('.rs-cell.rs-selected');
